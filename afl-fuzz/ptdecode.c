...
void decode_trace_tip_fast(unsigned char *data, size_t size, int coverage_kind, trace_callback func) {
	uint64_t next_ip;
...
inline static void process_packet(struct pt_packet *packet, int coverage_kind, trace_callback func) {
	// printf("packet type: %d\n", packet->type);
...
void decode_trace_tip_reference(unsigned char *trace_data, size_t trace_size,
	int coverage_kind, trace_callback func)
{
	...
	for (;;) {
		...
		for (;;) {
			...
			process_packet(&packet, coverage_kind, func);
		}
	}
...
void analyze_trace_full_reference(unsigned char *trace_data, size_t trace_size, struct pt_image *image, bool skip_first_bb, trace_callback func) {
	...
	for (;;) {
		...
		for (;;) {
			...
			//printf("next %llx\n", offset);
			if (prev_trace_offset == offset)
				if (stuck_count++ > 10000) {
					printf("WARNING: Trace decoding got stuck?\n");
					break; // wtf.
				}
			prev_trace_offset = offset;
			...
			if (!skip_next) {
				skip_next = false;
				if (block.iclass != ptic_error && block.iclass != ptic_other && !block.speculative)
				func(block.ip);
			...
			if (block.iclass == ptic_other || block.iclass == ptic_error) skip_next = true;
